...
public static class EnumerableExtensions
{
    ...
    public static bool IsNotNullOrEmpty<T>(this IEnumerable<T>? sequence) => sequence is not null && sequence.Any();
    ...
    /// <param name="sequence">元素序列。</param>
    /// <returns> 如果 <paramref name="sequence" />不是 <c>null</c> 并且元素数量不为0，则返回 <c>true</c>；否则，返回<c>false</c>。</returns>
    public static bool IsNotNullOrEmpty(this IEnumerable? sequence) => sequence is not null && !sequence.IsSequenceNullOrEmpty();
    ...
    public static bool IsNullOrEmpty(this IEnumerable? sequence) => sequence is null || sequence.IsSequenceNullOrEmpty();
    ...
    /// <summary>
    ///     将 <paramref name="sequence" /> 拼接为一个字符串，并且使用 <paramref name="separator" /> 作为分隔符。
    /// </summary>
    /// <typeparam name="T"><paramref name="sequence" /> 中的元素类型。</typeparam>
    /// <param name="sequence">原元素序列。</param>
    /// <param name="separator">分隔符，默认为","。</param>
    /// <returns> 拼接后的字符串。</returns>
    public static string? Join<T>(this IEnumerable<T>? sequence, string? separator = ",") => sequence == null ? null : string.Join(separator, sequence);
        ...
        string separator = ", ", bool putInBrackets = true)
    {
        ...
        while (enumerator.MoveNext())
        {
            ...
            {
                val = value is null ? "null" : value.ToString() ?? "null";
            }
